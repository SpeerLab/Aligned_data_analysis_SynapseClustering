...
for cur_path = 1:18
    ...
    for i = 1:numel(statsVwater_ss)
        ...
        B_ID = B_ID(B_ID>0);
        if numel(B_ID) > 1
            BID_comp = cat(1,BID_comp,B_ID);
            comp_ss = cat(1,comp_ss,statsVwater_ss(i));
            comp_id = cat(1,comp_id,ones(numel(BID_comp),1) * comp_count);
            comp_count = comp_count + 1;
        else
            BID_simp = cat(1,BID_simp,B_ID);
    ...
    for i = 1:numel(comp_ss)
        Weighted_centroid_1 = cat(1,Weighted_centroid_1,comp_ss(i).WeightedCentroid);
    end
    for i = 1:numel(comp_ss)
    ...
    for i = 1:size(dist_list_12,1)
        temp_list = dist_list_12(i,:);
        temp_list = sort(temp_list);
        min_dist_list_12(i) = temp_list(2);
    end
    clear BP BG bg2
    ...
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_sssss(BID_comp);
    area = [statsR.area_p16];
    tints = [statsR.tints_p16];
    volume = [statsR.volume_p16];
    num_null = numel(find(volume == 0));
    area = area(volume>0);
    tints = tints(volume>0);
    dist_nonnull = dist(volume>0);
    dist_null = dist(volume<=0);
    ...
    writematrix(dist_nonnull,[out_path 'dist_nonnull_p16.csv'],'WriteMode','append');
    writematrix(dist_null,[out_path 'dist_null_p16.csv'],'WriteMode','append');
    writematrix([num_null,numel(statsRwater_sssss)],[out_path 'Null_num_p16.csv'],'WriteMode','append');
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_sssss(BID_comp);
    area = [statsR.area_p32];
    tints = [statsR.tints_p32];
    volume = [statsR.volume_p32];
    num_null = numel(find(volume == 0));
    area = area(volume>0);
    tints = tints(volume>0);
    dist_nonnull = dist(volume>0);
    ...
    writematrix(dist_nonnull,[out_path 'dist_nonnull_p32.csv'],'WriteMode','append');
    writematrix(dist_null,[out_path 'dist_null_p32.csv'],'WriteMode','append');
    writematrix([num_null,numel(statsRwater_sssss)],[out_path 'Null_num_p32.csv'],'WriteMode','append');
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_sssss(BID_comp);
    area = [statsR.area_p48];
    tints = [statsR.tints_p48];
    volume = [statsR.volume_p48];
    num_null = numel(find(volume == 0));
    area = area(volume>0);
    tints = tints(volume>0);
    dist_nonnull = dist(volume>0);
    ...
    writematrix(dist_nonnull,[out_path 'dist_nonnull_p48.csv'],'WriteMode','append');
    writematrix(dist_null,[out_path 'dist_null_p48.csv'],'WriteMode','append');
    writematrix([num_null,numel(statsRwater_sssss)],[out_path 'Null_num_p48.csv'],'WriteMode','append');
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_sssss(BID_comp);
    area = [statsR.area_p64];
    tints = [statsR.tints_p64];
    volume = [statsR.volume_p64];
    num_null = numel(find(volume == 0));
    area = area(volume>0);
    tints = tints(volume>0);
    dist_nonnull = dist(volume>0);
    ...
    writematrix(dist_nonnull,[out_path 'dist_nonnull_p64.csv'],'WriteMode','append');
    writematrix(dist_null,[out_path 'dist_null_p64.csv'],'WriteMode','append');
    writematrix([num_null,numel(statsRwater_sssss)],[out_path 'Null_num_p64.csv'],'WriteMode','append');
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_sssss(BID_comp);
    area = [statsR.area_p80];
    tints = [statsR.tints_p80];
    volume = [statsR.volume_p80];
    num_null = numel(find(volume == 0));
    area = area(volume>0);
    tints = tints(volume>0);
    dist_nonnull = dist(volume>0);
    ...
    writematrix(dist_nonnull,[out_path 'dist_nonnull_p80.csv'],'WriteMode','append');
    writematrix(dist_null,[out_path 'dist_null_p80.csv'],'WriteMode','append');
    ...
    for i = 1:numel(comp_sn)
        Weighted_centroid_1 = cat(1,Weighted_centroid_1,comp_sn(i).WeightedCentroid);
    end
    for i = 1:numel(comp_sn)
    ...
    dist = min_dist_list_12(comp_id);
    statsR = statsRwater_ssssn(BID_comp);
    area = [statsR.area_p16];
    tints = [statsR.tints_p16];
    volume = [statsR.volume_p16];
    num_null = numel(find(volume == 0));
