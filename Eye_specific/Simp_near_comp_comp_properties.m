...
out_path = 'X:\Chenghang\4_Color\Complex_Syn\Simp_near_comp_comp_properties\';
...
for file_ID = 1:18
    ...
    for i = 1:numel(statsVwater_sn)
        ...
        B_ID = B_ID(B_ID>0);
        if numel(B_ID) > 1
            comp_sn = cat(1,volume_comp,statsVwater_sn(i).Volume1_0);
        else
            simp_sn = cat(1,volume_comp,statsVwater_sn(i).Volume1_0);
        end
    end
    Centroid_simp = [];
    Centroid_comp = [];
    for i = 1:numel(statsVwater_sn)
        ...
        B_ID = B_ID(B_ID>0);
        if numel(B_ID) > 1
            Centroid_comp = cat(1,Centroid_comp,statsVwater_sn(i).WeightedCentroid);
        else
            Centroid_simp = cat(1,Centroid_simp,statsVwater_sn(i).WeightedCentroid);
        end
    end

    Weighted_centroid_1 = Centroid_comp;
    Weighted_centroid_2 = Centroid_simp;
    ...
    save([out_path sprintf('%03d',file_ID) '.mat'],'dist_list_11_sorted','dist_list_12_sorted','dist_list_22_sorted','dist_list_11_sorted_new','dist_list_12_sorted_new','dist_list_22_sorted_new','Weighted_centroid_1_new','Weighted_centroid_2_new','volume_simp','volume_comp');
