...
for file_ID = 1:18
    ...
    for i = 1:numel(statsVwater_sn)
        ...
        if numel(B_ID) > 1
            for j = 1:numel(B_ID)
                comp = cat(1,comp,statsRwater_ssssn(B_ID(j)));
            end
        else 
            if numel(B_ID) == 1
                simp = cat(1,simp,statsRwater_ssssn(B_ID(1)));
            end
        end
    end
    ...
    for i = 1:numel(statsVwater_sn)
        ...
        B_ID(1) = [];
        if numel(B_ID) > 1
                comp_V = cat(1,comp_V,statsVwater_sn(i));
        else 
            if numel(B_ID) == 1
                simp_V = cat(1,simp_V,statsVwater_sn(i));
            end
        end
    end
    ...
    num_comp_V_list(file_ID) = numel(comp_V);
end
%save([data_path 'complex_denstiy.mat'],'num_comp_B_list','num_simp_B_list','num_simp_V_list','num_comp_V_list')
%%
disp(num_comp_B_list);
disp(num_simp_B_list);
disp(num_comp_V_list);
disp(num_simp_V_list);