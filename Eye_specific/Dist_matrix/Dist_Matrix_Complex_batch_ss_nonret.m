...
for file_ID = 1:18
    ...
    for i = 1:numel(statsVwater_sn)
        ...
        if numel(B_ID) > 1
            for j = 1:numel(B_ID)
                comp = cat(1,comp,statsVwater_sn(i));
            end
        else 
            if numel(B_ID) == 1
                simp = cat(1,simp,statsVwater_sn(i));
            end
        end
    end
    load([Result_folder '5_V_Syn\R_paired_3.mat'])
    clear statsRwater_ssss
    statsRwater_ssss = comp;
    statsRwater_sssn = statsRwater_sssn;
    ...
    save([data_path 'CTB_Pos_Simp_near\Comp_Neg_Simp_non\' sprintf('%03d',file_ID) '.mat'],'dist_list_11_sorted','dist_list_12_sorted','dist_list_22_sorted','dist_list_11_sorted_new','dist_list_12_sorted_new','dist_list_22_sorted_new');
